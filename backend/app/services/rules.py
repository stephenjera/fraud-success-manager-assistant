...
from app.api import errors as api_errors  # noqa: F401 - imported here to keep one error shape
...
def pin_insight(
    *,
    ...
    if row is None:
        raise api_errors.not_found(f"Revision {revision_id!r} not found under that message.")
    ins = _con().execute(
...
def patch_insight(insight_id: str, *, sql: str | None, explanation: str | None) -> dict[str, Any]:
...
def rule_has_backtest(rule_id: str) -> bool:
    ...
    n = _con().execute(
        "SELECT COUNT(*) FROM backtest_results WHERE rule_id=%s", (rule_id,)
    ).fetchone()[0]
    return int(n) > 0
...
def draft_rule(insight_id: str, *, title: str | None) -> dict[str, Any]:
    ...
    ins = _con().execute(
        "SELECT id::text, sql FROM insights WHERE id=%s", (insight_id,)
    ).fetchone()
    if ins is None:
        raise api_errors.not_found(f"Insight {insight_id!r} not found.")
    where = core_rules.derive_where_clause(ins[1])
    # Re-validate as defence-in-depth (the FSM can PATCH the clause later;
    # the draft still must be a legal clause or the FSM is stuck).
    canonical = core_rules.validate_where_clause(where)
    chosen_title = title or "Unnamed rule"
    row = _con().execute(
    ...
    assumptions = core_rules.assumptions_for(ins[1])
    rationale = "Rule proposed from this insight's pinned SQL. Edit the clause before backtesting."
    return {
        "rule_id": row[0],
        "source_insight_id": row[1],
        "title": row[2],
...
def get_rule(rule_id: str) -> dict[str, Any]:
    ...
    out["latest_backtest"] = (
        {"backtest_id": bt[0], "where_clause": bt[1], "window": bt[2], "created_at": bt[3]}
        if bt
        else None
    )
    out["deployment"] = (
        {"deployment_id": dep[0], "backtest_id": dep[1], "external_rule_id": dep[2], "deployed_at": dep[3]}
        if dep
        else None
    )
    return out
...
def patch_rule(rule_id: str, *, title: str | None, where_clause: str | None) -> dict[str, Any]:
...
def backtest_rule(rule_id: str, *, window: str | None = None) -> dict[str, Any]:
    ...
    dto = result.as_dto(rule_id=rule_id, window=window or "full", where_clause=rule["where_clause"])
...
__all__ = [
    "pin_insight", "get_insight", "list_insights", "patch_insight", "delete_insight",
    "draft_rule", "get_rule", "list_rules", "patch_rule", "delete_rule",
    "backtest_rule", "backtests_for_rule", "get_backtest",
    "approve", "reject", "deploy_rule", "deploy_status", "disable_rule",
]
