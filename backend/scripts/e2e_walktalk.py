    ...
    def invoke(self, state: dict, config: dict) -> dict:
        for handler in (config or {}).get("callbacks", []) or []:
            if hasattr(handler, "on_tool_start"):
                handler.on_tool_start({"name": "run_sql"}, '{"sql": "SELECT COUNT(*) FROM transactions"}', run_id="fake-tool-run")
...
def _fake_reader(sql_text: str) -> core_flags.Result:  # noqa: ARG001 - deterministic stand-in
    return core_flags.Result(columns=["total_transactions"], rows=[[1159966]], row_cap=100, truncated=False)
...
class StepFailure(Exception):
    ...
    def __init__(self, step: int, method: str, url: str, request: Any, status: int, body: Any) -> None:
        self.step = step
        self.method = method
        self.url = url
...
class Walk:
    ...
    def call(self, step: int, method: str, url: str, body: Any = None, *, expect: int | None = None) -> Any:
    ...
    def _has(body: Any, *keys: str) -> Any:
        if not isinstance(body, dict) or any(k not in body for k in keys):
            found = sorted(body.keys()) if isinstance(body, dict) else type(body).__name__
    ...
    def wait_until_terminal(self, cid: str, mid: str) -> Any:
        ...
        raise StepFailure(6, "GET", url, None, -2, {"error": "timed out waiting for the run to reach a terminal status"})
...
def run_walk(w: Walk) -> None:
    ...
    body = w.call(4, "POST", f"/v1/conversations/{cid}/messages", {"text": "How many transactions are there?"}, expect=201)
    ...
    if raw.status_code != 200:
        raise StepFailure(5, "GET", f"/v1/runs/{run_id}/events", None, raw.status_code, raw.text)
    events = w._sse(raw.text)
    names = [e for e, _ in events]
    if not names or names[0] != "run.start" or names[-1] != "run.done":
        raise AssertionError(f"SSE must open with run.start and close with run.done: {names}")
    ...
    if body["status"] != "success" or body["grounding"] is None or body["error"] is not None:
        raise AssertionError(f"message must be success + grounding + null error; status={body['status']}")
    w._has(body["grounding"], "sql", "explanation", "assumptions", "tables_and_joins_used", "flags")
    ...
    if LIVE:
        ...
        g = body["grounding"]
        tools = [f"{d.get('tool')}({d.get('args')})" for e, d in events if e == "tool_call.start"]
        print("\n--- LIVE LLM EVIDENCE (step 6: the real agent turn) ---")
        print(f"  tokens   : {tokens.get('tokens_in')} in / {tokens.get('tokens_out')} out  "
        ...
        if int(tokens.get("tokens_out", 0) or 0) == 0 and fake_marker in str(g.get("explanation")):
            raise AssertionError("the run used the FAKE LLM seam, not a real model (0 tokens + canned prose)")
        print("------------------------------------------------------------\n")
    ...
    body = w.call(8, "POST", f"/v1/conversations/{cid}/messages/{mid}/rerun", {"sql": fsm_sql}, expect=200)
    ...
    body = w.call(10, "POST", f"/v1/conversations/{cid}/insights",
                  {"message_id": mid, "revision_id": rev2, "sql": fsm_sql, "explanation": "High-value transactions."},
                  expect=201)
    ...
    if body["status"] != "draft":
        raise AssertionError(f"a freshly drafted rule must be 'draft', got {body['status']}")

    # edit-and-own, still legal (no backtest row yet)
    body = w.call(15, "PATCH", f"/v1/rules/{rule_id}", {"where_clause": "amount_usd_cents > 200000"}, expect=200)
    ...
    if body.get("where_clause") != "amount_usd_cents > 200000":
        raise AssertionError(f"PATCH must persist the new clause; got {body.get('where_clause')!r}")
    ...
    w._has(body, "backtest_id", "rule_id", "window", "where_clause",
           "labeled_only", "full_universe", "temporal_stability", "sample")
    for uni in ("labeled_only", "full_universe"):
        ...
        w._has(body[uni]["metrics"], "precision", "recall", "false_positive_rate", "baseline_fraud_rate", "lift")
        w._has(body[uni]["coverage"], "matched_count", "total_rows", "total_fraud", "support")
    if not isinstance(body["temporal_stability"], dict) or not body["temporal_stability"]:
    ...
    w._last = (16, "PATCH", f"/v1/rules/{rule_id}", {"where_clause": "amount_usd_cents > 999999999"})
    frozen = c.patch(f"/v1/rules/{rule_id}", json={"where_clause": "amount_usd_cents > 999999999"})
    if frozen.status_code != 409 or frozen.json()["error"]["code"] != "RULE_ILLEGAL_TRANSITION":
        raise StepFailure(16, "PATCH", f"/v1/rules/{rule_id}",
                          {"where_clause": "amount_usd_cents > 999999999"}, frozen.status_code, frozen.json())
    ...
    w._has(body, "backtest_id", "rule_id", "window", "labeled_only", "full_universe", "temporal_stability", "sample")

    # ----- Phase 6: approve -> deploy -> catalog ----------------------------
    body = w.call(19, "POST", f"/v1/rules/{rule_id}/approve",
    ...
    if body["status"] != "approved":
        raise AssertionError(f"approve must move the rule to 'approved', got {body['status']}")
...
def _report_failure(exc: StepFailure) -> int:
    ...
    snippet = exc.body if isinstance(exc.body, (str, list)) else json.dumps(exc.body, default=str, indent=2)
    print(f"  response:\n{(snippet or '')[:2000]}")
    print(bar + "\n")
    return 1
...
def main() -> int:
    if LIVE:
        print("LIVE mode: real LLM (Ollama) + real reference reader; shape-only asserts.")
        print("Note: the agent's own SQL is non-deterministic -- exact metric numbers\n"
    ...
    try:
        ...
        return _report_failure(StepFailure(step, method, url or "<shape check>", body, -2, str(exc)))
